...
interface SideComparisonCardProps {
  ...
  /** Whether to wrap the root in a motion.div with entrance animation */
  animated?: boolean;
  /** Base delay for staggered animations (default 0.2) */
  baseDelay?: number;
  eventType?: EventType;
}

export function SideComparisonCard({
  stats,
  members,
  animated = false,
  baseDelay = 0.2,
  ...
  const content = (
    <>
        ...
        <div className="relative h-3 rounded-full overflow-hidden flex" style={{ background: 'var(--groom-bg)' }}>
          {memberData.map((m, i) => (
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{ width: `${m.pct}%` }}
              transition={{ duration: 1, ease: [0.34, 1.56, 0.64, 1], delay: baseDelay + i * 0.05 }}
            />
          ))}
        </div>
        <div className="flex justify-between mt-1.5">
          {memberData.map((m) => (
            <span key={m.name} className="text-xs" style={{ color: m.color }}>{m.pct.toFixed(0)}%</span>
          ))}
        </div>
      </div>

      {/* Member stats grid */}
      <div className="grid gap-0 pb-5" style={{ gridTemplateColumns: memberData.map(() => 'minmax(0,1fr)').join(' auto ') }}>
        {memberData.map((m, i) => (
          <div key={m.name} className="contents">
            <motion.div
              ...
              transition={{ duration: 0.5, delay: baseDelay + 0.15 + i * 0.05 }}
            >
              <div
                className="w-9 h-9 rounded-xl flex items-center justify-center text-base font-heading font-bold"
              ...
              <div className="text-center min-w-0 w-full">
                ...
                <p className="font-heading font-bold text-sm leading-tight truncate" style={{ color: 'var(--stats-value)' }}>
                  {formatKRW(m.total)}
                </p>
              </div>
              <div className="w-full space-y-1.5 min-w-0">
                <div className="rounded-xl px-3 py-1.5 flex items-center justify-between gap-1 min-w-0" style={{ background: `${m.color}11` }}>
      ...
      >
        <span className="text-xs font-medium" style={{ color: 'var(--stats-label)' }}>합산 총액</span>
